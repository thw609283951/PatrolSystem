...
public class LoginPresenter extends BasePresenter<LoginActivity> {
    ...
    public void login(final String username, final String password){
        ServerModel.getInstance().loginServer(username,password)
                .doOnSubscribe(disposable -> putDisposable(disposable))
                .concatMap(myResponse -> {
                    UserModel.getInstance().setCurrUser(myResponse.getResultData());
                   return mIIMService.loginEaseIM(username,password,myResponse.getResultData());
                })
                .compose(RxSchedulers.io_main())
                .subscribe(new Observer<MyResponse>() {
                    @Override
                    public void onSubscribe(Disposable disposable) {
                        getView().showProgressDialog(R.string.Is_landing,disposable);
                    }

                    @Override
                    public void onNext(MyResponse value) {
                        getView().showSuccessToast(getView().getString(R.string.login_success));
